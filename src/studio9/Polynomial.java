...
public class Polynomial {
	...
	public String toString() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!result.isEmpty() && coeff > 0)
				result += " + "; // Add "+" for positive coefficients (except the first term)
			if (!result.isEmpty() && coeff < 0)
				result += " "; // Add space for negative coefficients

			if (power == 0) {
				result += coeff; // Constant term
			} else if (power == 1) {
				result += coeff + "x"; // Linear term. Power of 1.
			} else {
				result += coeff + "x^" + power; // Higher power terms
			}
		}
	...
	public Polynomial derivative() {
		...
		for (int i = 0; i < size - 1; i++) { // Skip the constant term
			double coeff = list.get(i);
			int power = size - 1 - i;
